            ...
            switch (type) {
                case 1:
                    if (ignoreVisibility) {
                        ...
                        return Insets.m7of(0, Math.max(rootStable.top, getSystemWindowInsets().top), 0, 0);
                    }
                    Insets rootStable2 = getSystemWindowInsets();
                    return Insets.m7of(0, rootStable2.top, 0, 0);
                case 2:
                    if (ignoreVisibility) {
                        Insets rootStable3 = getRootStableInsets();
                        Insets stable = getStableInsets();
                        return Insets.m7of(Math.max(rootStable3.left, stable.left), 0, Math.max(rootStable3.right, stable.right), Math.max(rootStable3.bottom, stable.bottom));
                    }
