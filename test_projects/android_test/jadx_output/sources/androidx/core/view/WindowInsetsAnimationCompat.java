...
public final class WindowInsetsAnimationCompat {
    ...
    public static class Impl21 extends Impl {
        ...
        static void setCallback(View view, Callback callback) {
            Object userListener = view.getTag(C0212R.id.tag_on_apply_window_listener);
            if (callback == null) {
                view.setTag(C0212R.id.tag_window_insets_animation_callback, null);
            ...
            view.setTag(C0212R.id.tag_window_insets_animation_callback, proxyListener);
            if (userListener == null) {
        ...
        static BoundsCompat computeAnimationBounds(WindowInsetsCompat targetInsets, WindowInsetsCompat startingInsets, int mask) {
            ...
            Insets lowerBound = Insets.m7of(Math.min(targetInsetsInsets.left, startingInsetsInsets.left), Math.min(targetInsetsInsets.top, startingInsetsInsets.top), Math.min(targetInsetsInsets.right, startingInsetsInsets.right), Math.min(targetInsetsInsets.bottom, startingInsetsInsets.bottom));
            Insets upperBound = Insets.m7of(Math.max(targetInsetsInsets.left, startingInsetsInsets.left), Math.max(targetInsetsInsets.top, startingInsetsInsets.top), Math.max(targetInsetsInsets.right, startingInsetsInsets.right), Math.max(targetInsetsInsets.bottom, startingInsetsInsets.bottom));
        ...
        static WindowInsets forwardToViewIfNeeded(View v, WindowInsets insets) {
            if (v.getTag(C0212R.id.tag_on_apply_window_listener) != null) {
                return insets;
            }
        ...
        static Callback getCallback(View child) {
            Object listener = child.getTag(C0212R.id.tag_window_insets_animation_callback);
