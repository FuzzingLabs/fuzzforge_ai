...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        ...
        TypedArray array = resources.obtainAttributes(attrs, C0212R.styleable.FontFamily);
        String authority = array.getString(C0212R.styleable.FontFamily_fontProviderAuthority);
        String providerPackage = array.getString(C0212R.styleable.FontFamily_fontProviderPackage);
        String query = array.getString(C0212R.styleable.FontFamily_fontProviderQuery);
        int certsId = array.getResourceId(C0212R.styleable.FontFamily_fontProviderCerts, 0);
        int strategy = array.getInteger(C0212R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int timeoutMs = array.getInteger(C0212R.styleable.FontFamily_fontProviderFetchTimeout, DEFAULT_TIMEOUT_MILLIS);
        String systemFontFamilyName = array.getString(C0212R.styleable.FontFamily_fontProviderSystemFontFamily);
        array.recycle();
    ...
    private static FontFileResourceEntry readFont(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        ...
        int i4;
        AttributeSet attrs = Xml.asAttributeSet(parser);
        TypedArray array = resources.obtainAttributes(attrs, C0212R.styleable.FontFamilyFont);
        if (array.hasValue(C0212R.styleable.FontFamilyFont_fontWeight)) {
            weightAttr = C0212R.styleable.FontFamilyFont_fontWeight;
        } else {
            weightAttr = C0212R.styleable.FontFamilyFont_android_fontWeight;
        }
        int weight = array.getInt(weightAttr, NORMAL_WEIGHT);
        if (array.hasValue(C0212R.styleable.FontFamilyFont_fontStyle)) {
            i = C0212R.styleable.FontFamilyFont_fontStyle;
        } else {
            i = C0212R.styleable.FontFamilyFont_android_fontStyle;
        }
        int styleAttr = i;
        boolean isItalic = 1 == array.getInt(styleAttr, 0);
        if (array.hasValue(C0212R.styleable.FontFamilyFont_ttcIndex)) {
            i2 = C0212R.styleable.FontFamilyFont_ttcIndex;
        } else {
            i2 = C0212R.styleable.FontFamilyFont_android_ttcIndex;
        }
        int ttcIndexAttr = i2;
        if (array.hasValue(C0212R.styleable.FontFamilyFont_fontVariationSettings)) {
            i3 = C0212R.styleable.FontFamilyFont_fontVariationSettings;
        } else {
            i3 = C0212R.styleable.FontFamilyFont_android_fontVariationSettings;
        }
        ...
        if (array.hasValue(C0212R.styleable.FontFamilyFont_font)) {
            i4 = C0212R.styleable.FontFamilyFont_font;
        } else {
            i4 = C0212R.styleable.FontFamilyFont_android_font;
        }
        int resourceAttr = i4;
