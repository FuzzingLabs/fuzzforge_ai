...
public class ItemTouchUIUtilImpl implements ItemTouchUIUtil {
    ...
    public void onDraw(Canvas c, RecyclerView recyclerView, View view, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        if (Build.VERSION.SDK_INT >= 21 && isCurrentlyActive) {
            Object originalElevation = view.getTag(C0433R.id.item_touch_helper_previous_elevation);
            if (originalElevation == null) {
                ...
                view.setTag(C0433R.id.item_touch_helper_previous_elevation, originalElevation2);
            }
        }
    ...
    public void clearView(View view) {
        if (Build.VERSION.SDK_INT >= 21) {
            Object tag = view.getTag(C0433R.id.item_touch_helper_previous_elevation);
            if (tag instanceof Float) {
                ViewCompat.setElevation(view, ((Float) tag).floatValue());
            }
            view.setTag(C0433R.id.item_touch_helper_previous_elevation, null);
        }
