...
public class ReactiveGuide extends View implements SharedValues.SharedValuesListener {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C0208R.styleable.ConstraintLayout_ReactiveGuide);
            int N = a.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0208R.styleable.ConstraintLayout_ReactiveGuide_reactiveGuide_valueId) {
                    this.mAttributeId = a.getResourceId(attr, this.mAttributeId);
                } else if (attr == C0208R.styleable.ConstraintLayout_ReactiveGuide_reactiveGuide_animateChange) {
                    this.mAnimateChange = a.getBoolean(attr, this.mAnimateChange);
                } else if (attr == C0208R.styleable.f63x2694048c) {
                    this.mApplyToConstraintSetId = a.getResourceId(attr, this.mApplyToConstraintSetId);
                } else if (attr == C0208R.styleable.f62xfdeff96) {
