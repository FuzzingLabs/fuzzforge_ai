...
public class MotionLabel extends View implements FloatLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C0208R.styleable.MotionLabel);
            int N = a.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0208R.styleable.MotionLabel_android_text) {
                    setText(a.getText(attr));
                } else if (attr == C0208R.styleable.MotionLabel_android_fontFamily) {
                    this.mFontFamily = a.getString(attr);
                } else if (attr == C0208R.styleable.MotionLabel_scaleFromTextSize) {
                    this.mBaseTextSize = a.getDimensionPixelSize(attr, (int) this.mBaseTextSize);
                } else if (attr == C0208R.styleable.MotionLabel_android_textSize) {
                    this.mTextSize = a.getDimensionPixelSize(attr, (int) this.mTextSize);
                } else if (attr == C0208R.styleable.MotionLabel_android_textStyle) {
                    this.mStyleIndex = a.getInt(attr, this.mStyleIndex);
                } else if (attr == C0208R.styleable.MotionLabel_android_typeface) {
                    this.mTypefaceIndex = a.getInt(attr, this.mTypefaceIndex);
                } else if (attr == C0208R.styleable.MotionLabel_android_textColor) {
                    this.mTextFillColor = a.getColor(attr, this.mTextFillColor);
                } else if (attr == C0208R.styleable.MotionLabel_borderRound) {
                    this.mRound = a.getDimension(attr, this.mRound);
                    if (Build.VERSION.SDK_INT >= 21) {
                        setRound(this.mRound);
                    }
                } else if (attr == C0208R.styleable.MotionLabel_borderRoundPercent) {
                    this.mRoundPercent = a.getFloat(attr, this.mRoundPercent);
                    if (Build.VERSION.SDK_INT >= 21) {
                        setRoundPercent(this.mRoundPercent);
                    }
                } else if (attr == C0208R.styleable.MotionLabel_android_gravity) {
                    setGravity(a.getInt(attr, -1));
                } else if (attr == C0208R.styleable.MotionLabel_android_autoSizeTextType) {
                    this.mAutoSizeTextType = a.getInt(attr, 0);
                } else if (attr == C0208R.styleable.MotionLabel_textOutlineColor) {
                    this.mTextOutlineColor = a.getInt(attr, this.mTextOutlineColor);
                    this.mUseOutline = true;
                } else if (attr == C0208R.styleable.MotionLabel_textOutlineThickness) {
                    this.mTextOutlineThickness = a.getDimension(attr, this.mTextOutlineThickness);
                    this.mUseOutline = true;
                } else if (attr == C0208R.styleable.MotionLabel_textBackground) {
                    this.mTextBackground = a.getDrawable(attr);
                    this.mUseOutline = true;
                } else if (attr == C0208R.styleable.MotionLabel_textBackgroundPanX) {
                    this.mBackgroundPanX = a.getFloat(attr, this.mBackgroundPanX);
                } else if (attr == C0208R.styleable.MotionLabel_textBackgroundPanY) {
                    this.mBackgroundPanY = a.getFloat(attr, this.mBackgroundPanY);
                } else if (attr == C0208R.styleable.MotionLabel_textPanX) {
                    this.mTextPanX = a.getFloat(attr, this.mTextPanX);
                } else if (attr == C0208R.styleable.MotionLabel_textPanY) {
                    this.mTextPanY = a.getFloat(attr, this.mTextPanY);
                } else if (attr == C0208R.styleable.MotionLabel_textBackgroundRotate) {
                    this.mRotate = a.getFloat(attr, this.mRotate);
                } else if (attr == C0208R.styleable.MotionLabel_textBackgroundZoom) {
                    this.mZoom = a.getFloat(attr, this.mZoom);
                } else if (attr == C0208R.styleable.MotionLabel_textureHeight) {
                    this.mTextureHeight = a.getDimension(attr, this.mTextureHeight);
                } else if (attr == C0208R.styleable.MotionLabel_textureWidth) {
                    this.mTextureWidth = a.getDimension(attr, this.mTextureWidth);
                } else if (attr == C0208R.styleable.MotionLabel_textureEffect) {
    ...
    private void setUpTheme(Context context, AttributeSet attrs) {
        ...
        theme.resolveAttribute(C0046R.attr.colorPrimary, typedValue, true);
        TextPaint textPaint = this.mPaint;
