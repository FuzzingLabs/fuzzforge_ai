...
public class MotionScene {
    ...
    public static class Transition {
        ...
        public static class TransitionOnClick implements View.OnClickListener {
            ...
            public TransitionOnClick(Context context, Transition transition, XmlPullParser parser) {
                ...
                TypedArray a = context.obtainStyledAttributes(Xml.asAttributeSet(parser), C0208R.styleable.OnClick);
                int N = a.getIndexCount();
                for (int i = 0; i < N; i++) {
                    int attr = a.getIndex(i);
                    if (attr == C0208R.styleable.OnClick_targetId) {
                        this.mTargetId = a.getResourceId(attr, this.mTargetId);
                    } else if (attr == C0208R.styleable.OnClick_clickAction) {
        ...
        private void fillFromAttributeList(MotionScene motionScene, Context context, AttributeSet attrs) {
            TypedArray a = context.obtainStyledAttributes(attrs, C0208R.styleable.Transition);
        ...
        private void fill(MotionScene motionScene, Context context, TypedArray a) {
            ...
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0208R.styleable.Transition_constraintSetEnd) {
                ...
                } else if (attr == C0208R.styleable.Transition_constraintSetStart) {
                ...
                } else if (attr == C0208R.styleable.Transition_motionInterpolator) {
                ...
                } else if (attr == C0208R.styleable.Transition_duration) {
                    int i2 = a.getInt(attr, this.mDuration);
                    this.mDuration = i2;
                    if (i2 < 8) {
                        this.mDuration = 8;
                    }
                } else if (attr == C0208R.styleable.Transition_staggered) {
                    this.mStagger = a.getFloat(attr, this.mStagger);
                } else if (attr == C0208R.styleable.Transition_autoTransition) {
                    this.mAutoTransition = a.getInteger(attr, this.mAutoTransition);
                } else if (attr == C0208R.styleable.Transition_android_id) {
                    this.mId = a.getResourceId(attr, this.mId);
                } else if (attr == C0208R.styleable.Transition_transitionDisable) {
                    this.mDisable = a.getBoolean(attr, this.mDisable);
                } else if (attr == C0208R.styleable.Transition_pathMotionArc) {
                    this.mPathMotionArc = a.getInteger(attr, -1);
                } else if (attr == C0208R.styleable.Transition_layoutDuringTransition) {
                    this.mLayoutDuringTransition = a.getInteger(attr, 0);
                } else if (attr == C0208R.styleable.Transition_transitionFlags) {
    ...
    public MotionScene(Context context, MotionLayout layout, int resourceID) {
        ...
        this.mConstraintSetMap.put(C0208R.id.motion_base, new ConstraintSet());
        this.mConstraintSetIdMap.put("motion_base", Integer.valueOf(C0208R.id.motion_base));
    }
    ...
    private void parseMotionSceneTags(Context context, XmlPullParser parser) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C0208R.styleable.MotionScene);
        int count = a.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = a.getIndex(i);
            if (attr == C0208R.styleable.MotionScene_defaultDuration) {
                int i2 = a.getInt(attr, this.mDefaultDuration);
                this.mDefaultDuration = i2;
                if (i2 < 8) {
                    this.mDefaultDuration = 8;
                }
            } else if (attr == C0208R.styleable.MotionScene_layoutDuringTransition) {
                this.mLayoutDuringTransition = a.getInteger(attr, 0);
            }
        }
        a.recycle();
    ...
    private void parseInclude(Context context, XmlPullParser mainParser) {
        TypedArray a = context.obtainStyledAttributes(Xml.asAttributeSet(mainParser), C0208R.styleable.include);
        int N = a.getIndexCount();
        for (int i = 0; i < N; i++) {
            int attr = a.getIndex(i);
            if (attr == C0208R.styleable.include_constraintSet) {
                int resourceId = a.getResourceId(attr, -1);
