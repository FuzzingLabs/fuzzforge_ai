...
class ClockFaceView extends RadialViewGroup implements ClockHandView.OnRotateListener {
    ...
    public ClockFaceView(Context context, AttributeSet attrs) {
        this(context, attrs, C0690R.attr.materialClockStyle);
    }
    ...
    public ClockFaceView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C0690R.styleable.ClockFaceView, defStyleAttr, C0690R.style.Widget_MaterialComponents_TimePicker_Clock);
        Resources res = getResources();
        ColorStateList colorStateList = MaterialResources.getColorStateList(context, a, C0690R.styleable.ClockFaceView_clockNumberTextColor);
        this.textColor = colorStateList;
        LayoutInflater.from(context).inflate(C0690R.layout.material_clockface_view, (ViewGroup) this, true);
        ClockHandView clockHandView = (ClockHandView) findViewById(C0690R.id.material_clock_hand);
        this.clockHandView = clockHandView;
        this.clockHandPadding = res.getDimensionPixelSize(C0690R.dimen.material_clock_hand_padding);
        ...
        int defaultBackgroundColor = AppCompatResources.getColorStateList(context, C0690R.color.material_timepicker_clockface).getDefaultColor();
        ColorStateList backgroundColor = MaterialResources.getColorStateList(context, a, C0690R.styleable.ClockFaceView_clockFaceBackgroundColor);
        ...
        this.valueAccessibilityDelegate = new AccessibilityDelegateCompat() { // from class: com.google.android.material.timepicker.ClockFaceView.2
            ...
            public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfoCompat info) {
                ...
                int index = ((Integer) host.getTag(C0690R.id.material_value_index)).intValue();
                if (index > 0) {
        ...
        this.minimumHeight = res.getDimensionPixelSize(C0690R.dimen.material_time_picker_minimum_screen_height);
        this.minimumWidth = res.getDimensionPixelSize(C0690R.dimen.material_time_picker_minimum_screen_width);
        this.clockSize = res.getDimensionPixelSize(C0690R.dimen.material_clock_size);
    }
    ...
    private void updateTextViews(int contentDescription) {
        ...
        for (int i = 0; i < Math.max(this.values.length, size); i++) {
            ...
            if (i >= this.values.length) {
                ...
                if (textView == null) {
                    textView = (TextView) inflater.inflate(C0690R.layout.material_clockface_textview, (ViewGroup) this, false);
                    this.textViewPool.put(i, textView);
                    addView(textView);
                }
                textView.setVisibility(0);
                textView.setText(this.values[i]);
                textView.setTag(C0690R.id.material_value_index, Integer.valueOf(i));
