...
public class MaterialContainerTransformSharedElementCallback extends SharedElementCallback {
    ...
    public View onCreateSnapshotView(Context context, Parcelable snapshot) {
        ...
        if (snapshotView != null && (weakReference = capturedSharedElement) != null && this.shapeProvider != null && (sharedElement = weakReference.get()) != null && (shapeAppearanceModel = this.shapeProvider.provideShape(sharedElement)) != null) {
            snapshotView.setTag(C0690R.id.mtrl_motion_snapshot_view, shapeAppearanceModel);
        }
        return snapshotView;
    }
    ...
    public void onSharedElementStart(List<String> sharedElementNames, List<View> sharedElements, List<View> sharedElementSnapshots) {
        if (!sharedElements.isEmpty() && !sharedElementSnapshots.isEmpty()) {
            sharedElements.get(0).setTag(C0690R.id.mtrl_motion_snapshot_view, sharedElementSnapshots.get(0));
        }
    ...
    public void onSharedElementEnd(List<String> sharedElementNames, List<View> sharedElements, List<View> sharedElementSnapshots) {
        if (!sharedElements.isEmpty() && (sharedElements.get(0).getTag(C0690R.id.mtrl_motion_snapshot_view) instanceof View)) {
            sharedElements.get(0).setTag(C0690R.id.mtrl_motion_snapshot_view, null);
        }
