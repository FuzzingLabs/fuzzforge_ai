...
public class ViewTransition {
    ...
    private void parseViewTransitionTags(Context context, XmlPullParser parser) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C0208R.styleable.ViewTransition);
        int count = a.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = a.getIndex(i);
            if (attr == C0208R.styleable.ViewTransition_android_id) {
                this.mId = a.getResourceId(attr, this.mId);
            } else if (attr == C0208R.styleable.ViewTransition_motionTarget) {
            ...
            } else if (attr == C0208R.styleable.ViewTransition_onStateTransition) {
                this.mOnStateTransition = a.getInt(attr, this.mOnStateTransition);
            } else if (attr == C0208R.styleable.ViewTransition_transitionDisable) {
                this.mDisabled = a.getBoolean(attr, this.mDisabled);
            } else if (attr == C0208R.styleable.ViewTransition_pathMotionArc) {
                this.mPathMotionArc = a.getInt(attr, this.mPathMotionArc);
            } else if (attr == C0208R.styleable.ViewTransition_duration) {
                this.mDuration = a.getInt(attr, this.mDuration);
            } else if (attr == C0208R.styleable.ViewTransition_upDuration) {
                this.mUpDuration = a.getInt(attr, this.mUpDuration);
            } else if (attr == C0208R.styleable.ViewTransition_viewTransitionMode) {
                this.mViewTransitionMode = a.getInt(attr, this.mViewTransitionMode);
            } else if (attr == C0208R.styleable.ViewTransition_motionInterpolator) {
            ...
            } else if (attr == C0208R.styleable.ViewTransition_setsTag) {
                this.mSetsTag = a.getResourceId(attr, this.mSetsTag);
            } else if (attr == C0208R.styleable.ViewTransition_clearsTag) {
                this.mClearsTag = a.getResourceId(attr, this.mClearsTag);
            } else if (attr == C0208R.styleable.ViewTransition_ifTagSet) {
                this.mIfTagSet = a.getResourceId(attr, this.mIfTagSet);
            } else if (attr == C0208R.styleable.ViewTransition_ifTagNotSet) {
                this.mIfTagNotSet = a.getResourceId(attr, this.mIfTagNotSet);
            } else if (attr == C0208R.styleable.ViewTransition_SharedValueId) {
                this.mSharedValueID = a.getResourceId(attr, this.mSharedValueID);
            } else if (attr == C0208R.styleable.ViewTransition_SharedValue) {
    ...
    void applyTransition(ViewTransitionController controller, MotionLayout layout, int fromId, ConstraintSet current, final View... views) {
        ...
        layout.updateState(C0208R.id.view_transition, current);
        layout.setState(C0208R.id.view_transition, -1, -1);
        MotionScene.Transition tmpTransition = new MotionScene.Transition(-1, layout.mScene, C0208R.id.view_transition, fromId);
    ...
    public /* synthetic */ void m6x14d7500(View[] views) {
        if (this.mSetsTag != -1) {
            for (View view : views) {
    ...
    private void updateTransition(MotionScene.Transition transition, View view) {
        ...
        if (keyFrames != null) {
            ...
            while (it.hasNext()) {
                Key key = it.next();
                keyFrames2.addKey(key.mo277clone().setViewId(id));
            }
