...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = startTimePoint.f55x + (f2 / 2.0f);
        float startCenterY = startTimePoint.f56y + (f4 / 2.0f);
        float endCenterX = endTimePoint.f55x + (f / 2.0f);
        float endCenterY = endTimePoint.f56y + (f3 / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f55x = (int) ((r15 + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f56y = (int) ((r3 + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 0;
        this.f55x = (int) (((startTimePoint.f55x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f56y = (int) (((startTimePoint.f56y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    public void setupRelative(MotionController mc, MotionPaths relative) {
        double dx = ((this.f55x + (this.width / 2.0f)) - relative.f55x) - (relative.width / 2.0f);
        double dy = ((this.f56y + (this.height / 2.0f)) - relative.f56y) - (relative.height / 2.0f);
        this.mRelativeToController = mc;
        this.f55x = (float) Math.hypot(dy, dx);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f56y = (float) (Math.atan2(dy, dx) + 1.5707963267948966d);
        } else {
            this.f56y = (float) Math.toRadians(this.mRelativeAngle);
        }
    }
    ...
    void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float pathVectorY = endCenterY - startCenterY;
        this.f55x = (int) ((r15 + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f56y = (int) ((r4 + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (f2 + (scaleX * scaleWidth));
        this.height = (int) (f4 + (scaleY * scaleHeight));
        this.mMode = 2;
        if (!Float.isNaN(c.mPercentX)) {
            this.f55x = (int) (c.mPercentX * ((int) (parentWidth - this.width)));
        }
        if (!Float.isNaN(c.mPercentY)) {
            this.f56y = (int) (c.mPercentY * ((int) (parentHeight - this.height)));
        }
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = (startTimePoint.width / 2.0f) + startTimePoint.f55x;
        float startCenterY = startTimePoint.f56y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f55x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f56y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f55x = (int) ((r11 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f56y = (int) ((r15 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 1;
        float endCenterY2 = startTimePoint.f55x;
        float f = (int) ((endCenterY2 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f55x = f;
        float f2 = (int) ((startTimePoint.f56y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.f56y = f2;
        this.f55x = f + normalX;
        this.f56y = f2 + normalY;
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean diffx = diff(this.f55x, points.f55x);
        boolean diffy = diff(this.f56y, points.f56y);
        int c = 0 + 1;
    ...
    void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        float delta_path;
        View view2 = view;
        float v_x2 = this.f55x;
        float v_y2 = this.f56y;
        float v_width = this.width;
    ...
    void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f55x, this.f56y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
