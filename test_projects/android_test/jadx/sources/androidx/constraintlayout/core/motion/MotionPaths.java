...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = startTimePoint.f20x + (f2 / 2.0f);
        float startCenterY = startTimePoint.f21y + (f4 / 2.0f);
        float endCenterX = endTimePoint.f20x + (f / 2.0f);
        float endCenterY = endTimePoint.f21y + (f3 / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f20x = (int) ((r15 + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f21y = (int) ((r3 + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 0;
        this.f20x = (int) (((startTimePoint.f20x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f21y = (int) (((startTimePoint.f21y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    public void setupRelative(Motion mc, MotionPaths relative) {
        double dx = ((this.f20x + (this.width / 2.0f)) - relative.f20x) - (relative.width / 2.0f);
        double dy = ((this.f21y + (this.height / 2.0f)) - relative.f21y) - (relative.height / 2.0f);
        this.mRelativeToController = mc;
        this.f20x = (float) Math.hypot(dy, dx);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f21y = (float) (Math.atan2(dy, dx) + 1.5707963267948966d);
        } else {
            this.f21y = (float) Math.toRadians(this.mRelativeAngle);
        }
    }
    ...
    void initScreen(int parentWidth, int parentHeight, MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float pathVectorY = endCenterY - startCenterY;
        this.f20x = (int) ((r15 + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f21y = (int) ((r4 + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (f2 + (scaleX * scaleWidth));
        this.height = (int) (f4 + (scaleY * scaleHeight));
        this.mMode = 2;
        if (!Float.isNaN(c.mPercentX)) {
            this.f20x = (int) (c.mPercentX * ((int) (parentWidth - this.width)));
        }
        if (!Float.isNaN(c.mPercentY)) {
            this.f21y = (int) (c.mPercentY * ((int) (parentHeight - this.height)));
        }
    ...
    void initPath(MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = (startTimePoint.width / 2.0f) + startTimePoint.f20x;
        float startCenterY = startTimePoint.f21y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f20x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f21y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f20x = (int) ((r11 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f21y = (int) ((r15 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 1;
        float endCenterY2 = startTimePoint.f20x;
        float f = (int) ((endCenterY2 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f20x = f;
        float f2 = (int) ((startTimePoint.f21y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.f21y = f2;
        this.f20x = f + normalX;
        this.f21y = f2 + normalY;
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean diffx = diff(this.f20x, points.f20x);
        boolean diffy = diff(this.f21y, points.f21y);
        int c = 0 + 1;
    ...
    void setView(float position, MotionWidget view, int[] toUse, double[] data, double[] slope, double[] cycle) {
        ...
        float v_x2 = this.f20x;
        float v_y2 = this.f21y;
    ...
    void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f20x, this.f21y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
