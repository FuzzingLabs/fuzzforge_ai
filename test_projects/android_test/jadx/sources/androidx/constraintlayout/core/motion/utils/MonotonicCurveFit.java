...
public class MonotonicCurveFit extends CurveFit {
    ...
    /* renamed from: mT */
    private double[] f32mT;
    private double[][] mTangent;

    /* renamed from: mY */
    private double[][] f33mY;
    ...
    public void getPos(double t, double[] v) {
        double[] dArr = this.f32mT;
        int n = dArr.length;
        int dim = this.f33mY[0].length;
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                getSlope(dArr[0], this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = this.f33mY[0][j] + ((t - this.f32mT[0]) * this.mSlopeTemp[j]);
                }
                return;
            }
            if (t >= dArr[n - 1]) {
                ...
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = this.f33mY[n - 1][j2] + ((t - this.f32mT[n - 1]) * this.mSlopeTemp[j2]);
                }
                return;
            }
        } else {
            if (t <= dArr[0]) {
                for (int j3 = 0; j3 < dim; j3++) {
                    v[j3] = this.f33mY[0][j3];
                }
                return;
            }
            if (t >= dArr[n - 1]) {
                for (int j4 = 0; j4 < dim; j4++) {
                    v[j4] = this.f33mY[n - 1][j4];
                }
                return;
            }
        }
    ...
    public void getPos(double t, float[] v) {
        ...
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                getSlope(dArr[0], this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = (float) (this.f33mY[0][j] + ((t - this.f32mT[0]) * this.mSlopeTemp[j]));
                }
                return;
            }
            if (t >= dArr[n - 1]) {
                getSlope(dArr[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = (float) (this.f33mY[n - 1][j2] + ((t - this.f32mT[n - 1]) * this.mSlopeTemp[j2]));
                }
                return;
            }
        } else {
            if (t <= dArr[0]) {
                for (int j3 = 0; j3 < dim; j3++) {
                    v[j3] = (float) this.f33mY[0][j3];
                }
                return;
            }
            if (t >= dArr[n - 1]) {
                for (int j4 = 0; j4 < dim; j4++) {
                    v[j4] = (float) this.f33mY[n - 1][j4];
                }
                return;
            }
        }
    ...
    public double getPos(double t, int j) {
        ...
        if (this.mExtrapolate) {
            if (t <= dArr[0]) {
                return this.f33mY[0][j] + ((t - dArr[0]) * getSlope(dArr[0], j));
            }
            if (t >= dArr[n - 1]) {
                return this.f33mY[n - 1][j] + ((t - dArr[n - 1]) * getSlope(dArr[n - 1], j));
            }
        } else {
            if (t <= dArr[0]) {
                return this.f33mY[0][j];
            }
            if (t >= dArr[n - 1]) {
                return this.f33mY[n - 1][j];
            }
        }
        for (int i = 0; i < n - 1; i++) {
            double[] dArr2 = this.f32mT;
            if (t == dArr2[i]) {
                return this.f33mY[i][j];
            }
            if (t < dArr2[i + 1]) {
                double h = dArr2[i + 1] - dArr2[i];
                double x = (t - dArr2[i]) / h;
