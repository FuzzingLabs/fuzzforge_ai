...
public class Scene {
    ...
    public static Scene getSceneForLayout(ViewGroup sceneRoot, int layoutId, Context context) {
        SparseArray<Scene> scenes = (SparseArray) sceneRoot.getTag(C0518R.id.transition_scene_layoutid_cache);
        if (scenes == null) {
            scenes = new SparseArray<>();
            sceneRoot.setTag(C0518R.id.transition_scene_layoutid_cache, scenes);
        }
    ...
    static void setCurrentScene(ViewGroup sceneRoot, Scene scene) {
        sceneRoot.setTag(C0518R.id.transition_current_scene, scene);
    }

    public static Scene getCurrentScene(ViewGroup sceneRoot) {
        return (Scene) sceneRoot.getTag(C0518R.id.transition_current_scene);
    }
