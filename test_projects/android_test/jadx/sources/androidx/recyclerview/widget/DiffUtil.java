...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f79x - o2.f79x;
            return cmpX == 0 ? o1.f80y - o2.f80y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f79x += range.oldListStart;
                snake.f80y += range.newListStart;
                Range left = rangePool.isEmpty() ? new Range() : rangePool.remove(rangePool.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f79x;
                    left.newListEnd = snake.f80y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f79x - 1;
                    left.newListEnd = snake.f80y;
                } else {
                    left.oldListEnd = snake.f79x;
                    left.newListEnd = snake.f80y - 1;
                }
                stack.add(left);
                if (!snake.reverse) {
                    range.oldListStart = snake.f79x + snake.size;
                    range.newListStart = snake.f80y + snake.size;
                } else if (snake.removal) {
                    range.oldListStart = snake.f79x + snake.size + 1;
                    range.newListStart = snake.f80y + snake.size;
                } else {
                    range.oldListStart = snake.f79x + snake.size;
                    range.newListStart = snake.f80y + snake.size + 1;
                }
                stack.add(range);
            } else {
                rangePool.add(range);
            ...
            for (int d = 0; d <= dLimit; d++) {
                for (int k = -d; k <= d; k += 2) {
                    ...
                    if (checkInFwd && k >= (delta - d) + 1 && k <= (delta + d) - 1 && forward[kOffset + k] >= backward[kOffset + k]) {
                        ...
                        outSnake.f79x = backward[kOffset + k];
                        outSnake.f80y = outSnake.f79x - k;
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                        ...
                        outSnake2.f79x = backward[kOffset + backwardK];
                        outSnake2.f80y = outSnake2.f79x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f79x != 0 || firstSnake.f80y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f79x + snake.size;
                int endY = snake.f80y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f79x + j;
                    int newItemPos = snake.f80y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f79x + snakeSize2;
                int endY2 = snake.f80y + snakeSize2;
                if (endX >= posOld2) {
                    endY = endY2;
                } else {
                    endY = endY2;
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f79x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f79x + i, 1, this.mCallback.getChangePayload(snake.f79x + i, snake.f80y + i));
                    }
                }
                posOld2 = snake.f79x;
                posNew2 = snake.f80y;
