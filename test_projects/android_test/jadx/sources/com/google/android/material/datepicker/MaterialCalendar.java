...
public final class MaterialCalendar<S> extends PickerFragment<S> {
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        if (MaterialDatePicker.isFullscreen(themedContext)) {
            int layout2 = C0690R.layout.mtrl_calendar_vertical;
            layout = layout2;
            orientation = 1;
        } else {
            int layout3 = C0690R.layout.mtrl_calendar_horizontal;
            layout = layout3;
            orientation = 0;
        }
        View root = themedInflater.inflate(layout, viewGroup, false);
        GridView daysHeader = (GridView) root.findViewById(C0690R.id.mtrl_calendar_days_of_week);
        ...
        this.recyclerView = (RecyclerView) root.findViewById(C0690R.id.mtrl_calendar_months);
        ...
        int columns = themedContext.getResources().getInteger(C0690R.integer.mtrl_calendar_year_selector_span);
        RecyclerView recyclerView = (RecyclerView) root.findViewById(C0690R.id.mtrl_calendar_year_selector_frame);
        ...
        if (root.findViewById(C0690R.id.month_navigation_fragment_toggle) != null) {
    ...
    static int getDayHeight(Context context) {
        return context.getResources().getDimensionPixelSize(C0690R.dimen.mtrl_calendar_day_height);
    }
    ...
    private void addActionsToMonthNavigation(View root, final MonthsPagerAdapter monthsPagerAdapter) {
        final MaterialButton monthDropSelect = (MaterialButton) root.findViewById(C0690R.id.month_navigation_fragment_toggle);
        ...
        ViewCompat.setAccessibilityDelegate(monthDropSelect, new AccessibilityDelegateCompat() { // from class: com.google.android.material.datepicker.MaterialCalendar.5
            ...
            public void onInitializeAccessibilityNodeInfo(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
                ...
                if (MaterialCalendar.this.dayFrame.getVisibility() == 0) {
                    string = MaterialCalendar.this.getString(C0690R.string.mtrl_picker_toggle_to_year_selection);
                } else {
                    string = MaterialCalendar.this.getString(C0690R.string.mtrl_picker_toggle_to_day_selection);
                }
                accessibilityNodeInfoCompat.setHintText(string);
            }
        });
        MaterialButton monthPrev = (MaterialButton) root.findViewById(C0690R.id.month_navigation_previous);
        monthPrev.setTag(NAVIGATION_PREV_TAG);
        MaterialButton monthNext = (MaterialButton) root.findViewById(C0690R.id.month_navigation_next);
        monthNext.setTag(NAVIGATION_NEXT_TAG);
        this.yearFrame = root.findViewById(C0690R.id.mtrl_calendar_year_selector_frame);
        this.dayFrame = root.findViewById(C0690R.id.mtrl_calendar_day_selector_frame);
        setSelector(CalendarSelector.DAY);
